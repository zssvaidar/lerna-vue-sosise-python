...
export default {
  ...
  name: 'Tree',
  emits: ['node-expand', 'node-collapse', 'update:expandedKeys', 'update:selectionKeys', 'node-select', 'node-unselect', 'node-chosen1', 'node-chosen2'],
  props: {
    value: {
      type: null,
      default: null
  ...
  methods: {
    ...
    isFilterMatched (node, { searchFields, filterText, strict }) {
      ...
      for (const field of searchFields) {
        const fieldValue = String(ObjectUtils.resolveFieldData(node, field)).toLocaleLowerCase(this.filterLocale)
